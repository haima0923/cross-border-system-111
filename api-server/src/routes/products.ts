...
router.post("/products/:id/manager-decision", async (req, res) => {
  ...
  const { id } = req.params;
  const { action, selectedOptionId, selectedSkuIds, comment, skuQuantities } = req.body;
  const operatorName = req.user!.name;
  ...
  // action === "approve"
  if (!selectedOptionId || !Array.isArray(selectedSkuIds) || selectedSkuIds.length === 0) {
    res.status(400).json({ error: "批准时必须指定 selectedOptionId 及至少一个 selectedSkuId" });
    return;
  }
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(sampleSkuLinesTable)
      ...
      .where(eq(sampleSkuLinesTable.sampleOptionId, selectedOptionId));
    ...
    await tx.update(sampleOptionsTable)
      ...
      .where(eq(sampleOptionsTable.id, selectedOptionId));

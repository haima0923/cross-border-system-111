...
router.get("/sample-sku-lines", async (req, res) => {
  ...
  const rows = await db.select().from(sampleSkuLinesTable).orderBy(asc(sampleSkuLinesTable.sampleOptionId), asc(sampleSkuLinesTable.createdAt));
...
router.get("/sample-sku-lines/:id", async (req, res) => {
  const [row] = await db.select().from(sampleSkuLinesTable).where(eq(sampleSkuLinesTable.id, req.params.id));
  if (!row) { res.status(404).json({ error: "SKU line not found" }); return; }
  res.json(serializeSkuLine(row as Record<string, unknown>));
});
...
router.put("/sample-sku-lines/:id/evaluation", async (req, res) => {
  ...
  const [existing] = await db.select().from(sampleSkuLinesTable).where(eq(sampleSkuLinesTable.id, id));
  if (!existing) {
...
router.post("/sample-sku-lines/:id/sampling-anomaly", async (req, res) => {
  ...
  // 获取SKU信息
  const [sku] = await db.select().from(sampleSkuLinesTable).where(eq(sampleSkuLinesTable.id, id));
  ...
  await db.update(sampleSkuLinesTable).set({
    ...
    anomalyReportedBy: operator || null,
...
router.post("/sample-sku-lines/:id/approve-sampling-anomaly", async (req, res) => {
  ...
  if (approved) {
    ...
    await db.update(sampleSkuLinesTable).set({
      ...
      anomalyHandledBy: operator || null,
      anomalyHandledAt: now,

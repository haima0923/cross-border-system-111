...
export default function AnalysisResult() {
  ...
  const handleResubmitAnalysis = () => {
    updateProduct(id, { status: 'pending_analysis' } as any, '重新提交分析', '员工补充信息后重新提交分析');
    setLocation('/workbench');
  };
  ...
  const needsEmployeeReview = product.status === 'analyzed_pending_review';
    ...
    <div className="max-w-5xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* History log */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-lg font-bold text-slate-800 mb-6">操作日志</h2>
            <div className="space-y-6">
              {product.historyLog?.map((log, i) => (
                <div key={i} className="flex gap-4 relative">
                  {i !== product.historyLog!.length - 1 && (
                    <div className="absolute top-8 left-2 bottom-0 w-0.5 bg-slate-100 -ml-px"></div>
                  )}
                  <div className="w-4 h-4 rounded-full bg-slate-200 border-4 border-white shadow-sm mt-1 z-10 shrink-0"></div>
                  <div>
                    <div className="flex items-baseline gap-2 mb-1">
                      <span className="font-semibold text-slate-800 text-sm">{log.actor}</span>
